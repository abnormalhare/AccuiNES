...
#include "_decode_rom.hpp"
    ...
    void CMP_I(CPU *cpu) {
        if (addr_immediate(cpu)) {
            ...
            cpu->P.c = (cpu->AI >= uint8_t(-cpu->BI)); // just pretend this is an ALU call
        }
    }
    void CMP_D(CPU *cpu) {
        if (addr_zero_read(cpu)) {
    ...
    void ISC_D(CPU *cpu) {
        if (addr_zero_rmw(cpu)) {
            ...
            cpu->AI = cpu->A;
            cpu->BI = ~cpu->ADD;
            cpu->callALU(SUM, callALU_flags(S_ALL));
            cpu->A = cpu->ADD;
        }
    }
    void ISC_DX(CPU *cpu) {
    ...
    void PLP(CPU *cpu) {
        switch (cpu->step) {
            ...
            case 3:
                cpu->readS();
                cpu->P.reg = (cpu->getDL() & ~0x30) | (cpu->P.reg & 0x30);
                break;
        }
    }
    ...
    void SAX_NX(CPU *cpu) {
        if (addr_indexed_indirect_write(cpu)) { cpu->setDOR(cpu->A & cpu->X); }
    }
